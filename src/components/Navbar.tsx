    ...
    <nav className="sticky top-0 z-50 bg-[#FDFCF8]/80 backdrop-blur-md border-b border-[#E5EAD7] px-4 py-3">
      <div className="container mx-auto flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          {!user ? (
            <Link to="/login" className="px-5 py-2 bg-[#4D7C0F] text-white rounded-xl font-bold text-sm hover:bg-[#3F6212] transition-colors">
              Sign In
            </Link>
          ) : (
            <>
              {user.role === 'farmer' && (
                <Link 
                  to="/farmer/interested-buyers" 
                  className="relative p-2 text-[#5B6D44] hover:text-[#4D7C0F] transition-colors"
                >
                  <Bell className="w-5 h-5" />
                  {MOCK_BUYER_LEADS.filter(b => b.status === 'pending' && b.farmerId === user.id).length > 0 && (
